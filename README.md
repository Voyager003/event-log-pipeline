...
Step 1에서 만든 이벤트 생성기는 Kubernetes에 배포한다고 가정하고 `Job`, `ConfigMap`, `Secret` manifest를 작성했습니다.

현재 이벤트 생성기는 외부 요청을 계속 받는 서버라기보다 실행 후 이벤트를 생성하고 저장하는 배치성 작업에 가깝습니다.

그래서 `Deployment`보다 `Job`을 중심으로 구성했습니다.

- Job

`Job`은 Pod를 실행해 정해진 작업을 수행하고, 작업이 끝나면 종료되는 리소스입니다.

이벤트 생성기는 실행되면 이벤트를 생성하고 PostgreSQL에 저장한 뒤 종료되므로 `Job`이 가장 적합하다고 판단했습니다.

- ConfigMap

`ConfigMap`은 이벤트 생성 건수, heartbeat 간격, DB URL 같은 환경별 설정값을 관리합니다.

애플리케이션 이미지 안에 설정을 고정하지 않고, 실행 환경에 따라 값을 바꿀 수 있도록 분리하기 위함입니다.

- Secret

`Secret`은 DB 계정과 비밀번호처럼 민감한 값을 관리하기 위해 사용합니다.

DB 접속 정보가 manifest나 이미지에 직접 들어가지 않도록 분리하는 것이 필요하다고 판단했습니다.

실제 운영 환경에서는 `k8s/secret.example.yaml`의 값을 그대로 사용하지 않고, 클러스터 환경에 맞게 별도 Secret을 생성해야 합니다.

`Service`는 작성하지 않았습니다. 이벤트 생성기 앱은 외부 요청을 받는 API 서버가 아니라 `Job Pod -> RDS PostgreSQL` 방향으로 데이터를 저장하는 작업이기 때문에 고정 네트워크 엔드포인트가 필요하지 않다고 판단했습니다.
